...
// import {
//   ALL_LEVELS, MONO, SHIFT_ORDER, TOTAL_COLS,
//   type Employee, type FilterState, type ShiftCode, SHIFT_COLORS,
// } from "./Gridtypes";
...
// import { alpha } from "@mui/material";
// import type { SxProps, Theme } from "@mui/material/styles";
// import type { FutureWeekRow } from "../api/rosterGenerationApiSlice";
// import { ALL_LEVELS, API_SHIFT_MAP, MONO, SHIFT_ORDER, TOTAL_COLS, type Employee, type FilterState, type ShiftCode, SHIFT_COLORS } from "../types/Gridtypes";
...
// export function buildFromApi(rows: FutureWeekRow[]): {
//   emps: Employee[];
//   grid: Record<string, ShiftCode[]>;
//   allRoles: string[];
// } {
...
//     role: row.roleCode,
//     level: (ALL_LEVELS.includes(row.jobLevel as any) ? row.jobLevel : "L1") as Employee["level"],
//   }));

//   const grid: Record<string, ShiftCode[]> = {};
//   rows.forEach((row) => {
//     grid[row.olmid] = [
//       toShiftCode(row.W7D1), toShiftCode(row.W7D2), toShiftCode(row.W7D3),
//       toShiftCode(row.W7D4), toShiftCode(row.W7D5), toShiftCode(row.W7D6),
...
// export const computeWeekStart = (rows: FutureWeekRow[]): Date => {
//   const today = new Date();
//   if (rows.length > 0 && rows[0].isoYear && rows[0].isoWeek) {
//     const jan4 = new Date(rows[0].isoYear, 0, 4);
//     return addDays(mondayOf(jan4), (rows[0].isoWeek - 1) * 7);
//   }
...
// export const colCounts = (grid: Record<string, ShiftCode[]>, emps: Employee[], idx: number) =>
//   emps.reduce((acc, e) => {
//     const c = grid[e.id]?.[idx];
//     if (c) acc[c]++;
//     return acc;
...
// export const workingTotal = (c: Record<ShiftCode, number>) =>
//   SHIFT_ORDER.filter((x) => x !== "OFF").reduce((s, x) => s + c[x], 0);
...
// ): Employee[] {
//   const result = emps.filter((e) => {
//     if (f.query && !`${e.name} ${e.id} ${e.role}`.toLowerCase().includes(f.query.toLowerCase()))
//       return false;
//     if (f.levels.length && !f.levels.includes(e.level)) return false;
//     if (f.roles.length && !f.roles.includes(e.role)) return false;
//     const row = grid[e.id] ?? [];
...
//   return result.sort((a, b) => {
//     const rA = grid[a.id] ?? [], sA = empSummary(rA);
//     const rB = grid[b.id] ?? [], sB = empSummary(rB);
//     let vA: string | number = "";
//     let vB: string | number = "";
//     switch (f.sortField) {
//       case "name":  vA = a.name;  vB = b.name; break;
...
//       case "night": vA = sA.n;    vB = sB.n;   break;
//       case "off":   vA = sA.off;  vB = sB.off; break;
//       case "load":  vA = sA.work / TOTAL_COLS; vB = sB.work / TOTAL_COLS; break;
//     }
...
//   textAlign: "left",
//   whiteSpace: "nowrap",
//   boxShadow: isDark ? "4px 0 12px -4px rgba(0,0,0,0.4)" : "4px 0 10px -4px rgba(13,27,42,0.1)",
//   borderRight: `1px solid ${theme.palette.divider}`,
// });
...
// export const headerBgSx = (isDark: boolean): SxProps<Theme> => ({
//   background: isDark ? "#1A2436" : "#F4F7FB",
//   position: "sticky",
//   top: 0,
//   zIndex: 3,
...
// export const wkendSx = (isDark: boolean): SxProps<Theme> => ({
//   background: isDark ? "rgba(255,255,255,0.015)" : "rgba(0,0,0,0.012)",
// });

// export const wsepSx = (theme: Theme): SxProps<Theme> => ({
//   borderLeft: `2px solid ${alpha(theme.palette.text.primary, 0.1)}`,
// });
