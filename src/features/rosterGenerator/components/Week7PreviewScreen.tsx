...
export const emptyCounts = () =>
  SHIFT_ORDER.reduce((a, c) => ({ ...a, [c]: 0 }), {});
export const colCounts = (grid, emps, index) =>
  emps.reduce((a, e) => {
    const c = grid[e.id]?.[index];
    if (c) a[c]++;
    return a;
  }, emptyCounts());
...
export const workingTotal = (counts) =>
  SHIFT_ORDER.filter((c) => c !== "OFF").reduce((s, c) => s + counts[c], 0);
  ...
  result.sort((a, b) => {
    ...
    switch (f.sortField) {
      ...
      case "off":
        ...
        break;
      case "load":
        vA = sA.work / totalCols;
        vB = sB.work / totalCols;
        break;
      default:
        vA = a.name;
        vB = b.name;
    ...
    <Collapse in={open}>
      ...
      >
        ...
        >
                ...
                return (
                  <Box
                    ...
                    onClick={() =>
                      update({
                        shiftCodes: active
                          ? local.shiftCodes.filter((x) => x !== code)
          ...
          <Box>
            ...
            <Slider
              ...
              onChange={(_, v) => update({ workRange: v })}
              min={0}
              max={totalCols}
            ...
            <Slider
              ...
              onChange={(_, v) => update({ nightRange: v })}
              min={0}
              max={totalCols}
              valueLabelDisplay="auto"
              size="small"
        ...
        >
          ...
          {["name", "id", "role", "level", "work", "night", "off", "load"].map(
            (field) => (
              <Chip
                key={field}
        ...
        {SHIFT_ORDER.map((code) => (
          <Button
            ...
            sx={{
              ...
              bgcolor:
                brush === code
                  ? alpha(SHIFT_COLORS[code].solid, 0.12)
                  : "transparent",
              color:
                brush === code ? SHIFT_COLORS[code].text : "text.secondary",
              boxShadow:
                brush === code
                  ? `0 0 0 3px ${alpha(SHIFT_COLORS[code].solid, 0.15)}`
                  : "none",
              transition: "all 0.15s ease",
              "&:hover": {
                borderColor: SHIFT_COLORS[code].solid,
                bgcolor: alpha(SHIFT_COLORS[code].solid, 0.08),
              },
            }}
          >
            {code}
    ...
    >
      ...
      <DialogContent>
              ...
              <Card
                ...
                sx={{
                  ...
                  "&:hover": {
                    ...
                    boxShadow: `0 4px 16px ${alpha(c.solid, 0.15)}`,
                  },
                }}
              >
                <ShiftBadge code={code} />
        ...
        <Alert severity="info" sx={{ borderRadius: "10px" }}>
          <strong>Busiest day:</strong> {busiest?.label ?? "N/A"} with{" "}
...
export function ExcelUploadModal({ open, title, subtitle, onClose, onImport }) {
  ...
  return (
    ...
    >
      <DialogTitle
        sx={{ fontWeight: 750, fontSize: 18, pb: 1, letterSpacing: "-0.02em" }}
      >
        {title}
      </DialogTitle>
      <DialogContent>
...
export function ValidationPanel({ grid, emps }) {
  ...
  return (
    <Card
      variant="outlined"
      sx={{ borderRadius: "12px", p: 2, borderColor: "divider" }}
    >
      <Stack direction="row" gap={1.5} flexWrap="wrap" alignItems="center">
        <Chip
          ...
          sx={{ fontWeight: 650, height: 28, borderRadius: "8px" }}
        />
        <Chip
                    ...
                    >
                      ...
                      <div
                        style={{
                          fontSize: 9.5,
                          fontWeight: 600,
                          fontFamily: MONO.split(",")[0].replace(/'/g, ""),
                          color: "inherit",
                          opacity: 0.7,
                          marginTop: 1,
                        }}
...
export function gridTableSx(theme) {
  ...
  return {
    ...
    "& tbody tr:hover td": {
      background: isDark ? "rgba(255,255,255,0.025)" : "rgba(13,27,42,0.025)",
    },
    ...
    "& .gbtn": {
      ...
      transition:
        "box-shadow 0.12s ease, border-color 0.12s ease, filter 0.12s ease",
    },
