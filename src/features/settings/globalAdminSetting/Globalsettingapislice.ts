...
/**
 * Returned by GET /global-settings/permissions/dropdown/permissions
 * — only id + name; permissionCode is derived client-side from the name
 * (e.g. "View" → "VIEW", "Sub Approve" → "SUB_APPROVE").
 */
export interface PermissionModel {
  permissionId: number;
  permissionName: string;
  permissionCode?: string; // optional — not returned by the catalog endpoint
}
...
export interface RolePermissionViewModel {
  ...
  subModuleId: number;
  subModuleName: string;
  /** Array of granted permissions (direct from API – NOT a JSON string) */
...
export const globalSettingApiSlice = api.injectEndpoints({
  endpoints: (builder) => ({
    ...
    // ── Dropdown: Permission catalog (View, Create, Update, …) ─
    // NOTE: endpoint is /permissions, NOT /permission-types
    getPermissionTypes: builder.query<PermissionModel[], void>({
      query: () => ({
        url: "/global-settings/permissions/dropdown/permissions",
        method: "GET",
      }),
    }),
