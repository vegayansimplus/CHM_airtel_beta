...
const isPendingAction = isAnyOf(
  ...Object.values(apiSlice.endpoints).map((endpoint: any) => endpoint.matchPending)
);
...
const isFulfilledOrRejectedAction = isAnyOf(
  ...Object.values(apiSlice.endpoints).map((endpoint: any) => endpoint.matchFulfilled),
  ...Object.values(apiSlice.endpoints).map((endpoint: any) => endpoint.matchRejected)
);
